...
export function RuleSetForm({ open, ruleSet, series, components, onSave, onClose }: Props) {
  ...
  const [template, setTemplate] = useState<TemplateSlot[]>(
    ruleSet?.dayTemplate ? JSON.parse(ruleSet.dayTemplate) : []
  );
  ...
  function addTemplateSlot(type: TemplateSlot["type"]) {
    setTemplate((t) => [...t, { type }]);
  }

  function removeTemplateSlot(i: number) {
    setTemplate((t) => t.filter((_, idx) => idx !== i));
  }

  function updateTemplateSlot(i: number, patch: Partial<TemplateSlot>) {
    setTemplate((t) => t.map((s, idx) => idx === i ? { ...s, ...patch } : s));
  }
                ...
                <div key={i} className="flex items-center gap-2 bg-muted rounded-md px-2 py-1.5">
                  ...
                  {slot.type === "fixed" ? (
                    <select
                      ...
                      onChange={(e) => updateTemplateSlot(i, { componentId: e.target.value })}
                  ...
                  <button type="button" onClick={() => removeTemplateSlot(i)} className="text-destructive hover:text-destructive/80 shrink-0">
